...
#define MIB_TxExcessiveCconst unsigned char ollision    0x1D         /**< # of transmitted excessive collisions */
 ...
 *****************************************************************************/
void KSZ8851SNL_MacAddressGet(uint8_t *aMacAddress)
{
  int i;


  EFM_ASSERT(aMacAddress != NULL);

  /* set the first 3 bytes given by the EM MAC Address space */
  aMacAddress[0] = HIGH_QMU_MAC_H;
  aMacAddress[1] = HIGH_QMU_MAC_L;
  aMacAddress[2] = MID_QMU_MAC_H;
  /* set the next 3 bytes given by the CMU unique ID */

  for (i = 0; i < 3; i++) {
    aMacAddress[5 - i] = (DEVINFO->UNIQUEL & (BYTE_MASK << i * BYTE_SIZE)) >> i * BYTE_SIZE;
  }
}
