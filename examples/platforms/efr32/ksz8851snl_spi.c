...
/*(USART 1, Location #1) */
#define ETH_CS_PIN      11U                      /**< SPI CHIP SELECT Pin */
#define ETH_CS_PORT      gpioPortB              /**< SPI CHIP SELECT Port */

#define ETH_RST_PIN		5U
#define ETH_RST_PORT 	gpioPortA
...
void KSZ8851SNL_On(void) {
	GPIO_PinOutSet(ETH_RST_PORT, ETH_RST_PIN);

}

void KSZ8851SNL_Off(void) {
	GPIO_PinOutClear(ETH_RST_PORT, ETH_RST_PIN);
...
void KSZ8851SNL_SPI_Init(void) {
	...
	initData.portLocationRx = PORTIO_USART1_RX_LOC;
	initData.portLocationTx = PORTIO_USART1_TX_LOC;
	initData.portLocationClk = PORTIO_USART1_CLK_LOC;
	initData.portLocationCs = PORTIO_USART1_CS_LOC;
	...
	GPIO_PinModeSet(ETH_RST_PORT, ETH_RST_PIN, gpioModePushPull, 1);
 ...
 *****************************************************************************/
void KSZ8851SNL_SPI_Transmit(int numBytes, const uint8_t * data) {
	Ecode_t result;
 ...
 *****************************************************************************/
void KSZ8851SNL_SPI_Receive(int numBytes, uint8_t * buffer) {
	Ecode_t result;
	int remaining = numBytes;
...
void KSZ8851SNL_SPI_WriteRegister(uint8_t reg, uint16_t value) {
	 ...
	 * reg=6 most significant bits of the register address
	 */
	txBuffer[0] = OPCODE_REG_WRITE
			| (BYTE_ENABLE << (BYTE_ENABLE_SHIFT + (reg & REG_MASK)));
	txBuffer[0] |= reg >> ADDRESS_MS2B_POS;
	txBuffer[1] = (reg << ADDRESS_SHIFT) & 0xf0;
