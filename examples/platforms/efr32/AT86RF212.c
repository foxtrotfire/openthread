...
void AT86RF212_Data_Req(uint8_t *data, uint8_t size)
{
    ...
    for (uint8_t i = 0; i < size; i++) AT86RF212_SPI_WriteByte(data[i]);
...
void PHY_EncryptReq(uint8_t *text, uint8_t *key)
{
    ...
    for (uint8_t i = 0; i < AES_BLOCK_SIZE; i++) AT86RF212_SPI_WriteByte(key[i]);

    AT86RF212_SPI_WriteByte(RF_CMD_SRAM_W);
    AT86RF212_SPI_WriteByte(AES_CTRL_REG);
    AT86RF212_SPI_WriteByte((0 << AES_CTRL_MODE) | (0 << AES_CTRL_DIR));
    for (uint8_t i = 0; i < AES_BLOCK_SIZE; i++) AT86RF212_SPI_WriteByte(text[i]);
    ...
    for (uint8_t i = 0; i < AES_BLOCK_SIZE; i++) text[i] = AT86RF212_SPI_WriteByte(0);
}
#endif
...
void AT86RF212_Wait_State(uint8_t state)
{
    while (state != (AT86RF212_SPI_ReadRegister(TRX_STATUS_REG) & TRX_STATUS_MASK))
        ;
}
